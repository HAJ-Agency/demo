...
const edit = (props) => {
   ...
   React.useEffect(() => {
      ...
      if (!mainClassName) {
         setAttributes({
            mainClassName: `ingredients-archive swiper block-editor-block-list__block wp-block ${blockProps.className} `,
         });
      }
   }, []);
      ...
      <>
         <InspectorControls>
            <PanelBody title="Inställningar">
               ...
               />
               <RangeControl
                  label="Synliga Inlägg Platta"
                  value={postsPerViewTablet}
                  onChange={(val) => setAttributes({ postsPerViewTablet: val })}
                  min={1}
                  max={5}
               />
               <RangeControl
         ...
         <ServerSideRender
            block="tiburon/ingredients-archive"
